...
darkcorr_dlg.cpp (C-Munipack project)
...
CDarkCorrDlg::CDarkCorrDlg(GtkWindow *pParent):m_pParent(pParent)
{
	GtkWidget *vbox;
	GSList *group;

	// Dialog with buttons
	m_pDlg = gtk_file_chooser_dialog_new("Dark correction", pParent, GTK_FILE_CHOOSER_ACTION_OPEN,
	...
	// Dialog icon
	gchar *icon = get_icon_file("darkcorr");
...
CDarkCorrDlg::~CDarkCorrDlg()
{
	gtk_widget_destroy(m_pDlg);
}

void CDarkCorrDlg::Execute()
{
	int res;
	char msg[256];
	char *path;
	...
	m_Preview.Clear();
	path = g_Project->GetStr("DarkCorr", "File", NULL);
	if (!path)
		path = CConfig::GetStr("DarkCorr", "File", NULL);
	...
	m_DarkFile = gtk_file_chooser_get_filename(GTK_FILE_CHOOSER(m_pDlg));
	g_Project->SetStr("DarkCorr", "File", m_DarkFile);
	CConfig::SetStr("DarkCorr", "File", m_DarkFile);
	...
	// Process files
	if (m_FileList) {
		CProgressDlg pDlg(m_pParent, "Processing DARK correction");
...
bool CDarkCorrDlg::SelectFrame(char **fpath)
{
	// Selection
	...
	if (*fpath && (*fpath)[0]!=0) {
		...
		gchar *path = g_Project->GetStr("DarkCorr", "File", NULL);
		if (!path)
			path = CConfig::GetStr("DarkCorr", "File", NULL);
		if (path && gtk_file_chooser_set_filename(GTK_FILE_CHOOSER(m_pDlg), path))
			m_Preview.Update(path);
	...
	if (gtk_dialog_run(GTK_DIALOG(m_pDlg))==GTK_RESPONSE_ACCEPT) {
		...
		g_Project->SetStr("DarkCorr", "File", path);
		CConfig::SetStr("DarkCorr", "File", path);
...
gboolean CDarkCorrDlg::foreach_all_files(GtkTreeModel *model, GtkTreePath *path, 
...
void CDarkCorrDlg::foreach_sel_files(GtkTreeModel *model, GtkTreePath *path, 
							   GtkTreeIter *iter, GList **pList)
{
	GtkTreeRowReference *rowref = gtk_tree_row_reference_new(model, path);
	*pList = g_list_append(*pList, rowref);
...
void CDarkCorrDlg::response_dialog(GtkDialog *pDlg, gint response_id, CDarkCorrDlg *pMe)
{
	if (!pMe->OnResponseDialog(response_id))
		g_signal_stop_emission_by_name(pDlg, "response");
}

bool CDarkCorrDlg::OnResponseDialog(gint response_id)
{
	gchar *fname, *f;
	...
	case GTK_RESPONSE_ACCEPT:
		...
		if (!fname || !g_file_test(fname, G_FILE_TEST_IS_REGULAR)) {
			ShowError(GTK_WINDOW(m_pDlg), "Choose a dark correction frame");
			g_free(fname);
			return false;
		}
	...
	case GTK_RESPONSE_HELP:
		// Show context help
		g_MainWnd->ShowHelp(GTK_WINDOW(m_pDlg), IDH_DARK_CORRECTION);
		return false;
	}
	return true;
}

int CDarkCorrDlg::ExecuteProc(CProgressDlg *sender, void *userdata)
{
	return ((CDarkCorrDlg*)userdata)->ProcessFiles(sender);
}

int CDarkCorrDlg::ProcessFiles(CProgressDlg *sender)
{
	int frameid, res;
	gchar *fpath, msg[128];
	GtkTreePath *path;
	...
	sender->Print("------ Dark correction ------");

	res = dark.Init(sender, m_DarkFile);
	if (res==0) {
		g_Project->SetOrigDarkFile(m_DarkFile);
...
void CDarkCorrDlg::update_preview(GtkFileChooser *widget, CDarkCorrDlg *pMe)
{
	pMe->OnUpdatePreview(widget);
}

void CDarkCorrDlg::OnUpdatePreview(GtkFileChooser *widget)
{
...
void CDarkCorrDlg::selection_changed(GtkFileChooser *widget, CDarkCorrDlg *pMe)
{
	pMe->OnSelectionChanged(widget);
}

void CDarkCorrDlg::OnSelectionChanged(GtkFileChooser *widget)
{
	UpdateControls();
}

void CDarkCorrDlg::UpdateControls(void)
{
	bool ok;
