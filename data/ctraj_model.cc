...
int main(int argc, char *argv[]) {
  ...
  for (long i=0; i<ninit; i++) {
    ...
    for (ind_type j=1; j<=nt[i]; j++) result[i][j]=result[i][0]+ndim*j;
  ...
  for (double i=tind0min; i<=tind0max; ) {
	...
	for (int k=1; k<=nt1; k++) hemi[j][tind[j]+k]=hemi[j][tind[j]];
        hemi[j][tind[j]+nt1]=vfield->fix(hemi[j][tind[j]+nt1], tind0[j]+tstep[j]*tind[j], 
			result[j][tind[j]+nt1]);
        //tcoord_fix(result[j][n+nt1][0], result[j][n+nt1][1], hemi[j][n+nt1]);
        //do the integration:
	tind[j]+=nt1;
        //printf("x");
      }
