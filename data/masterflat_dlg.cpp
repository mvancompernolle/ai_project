...
masterflat_dlg.cpp (C-Munipack project)
...
CMasterFlatDlg::CMasterFlatDlg(GtkWindow *pParent):m_pParent(pParent), m_InFiles(0), 
	m_FileList(NULL), m_FilePath(NULL)
{
	...
	// Dialog with buttons
	m_pDlg = gtk_file_chooser_dialog_new("Master flat frame", pParent, 
	...
	gtk_dialog_set_tooltip_by_response(GTK_DIALOG(m_pDlg), GTK_RESPONSE_ACCEPT, "Create a master flat frame and save it to the specified file");
	...
	// Dialog icon
	gchar *icon = get_icon_file("masterflat");
...
CMasterFlatDlg::~CMasterFlatDlg()
{
	g_free(m_FilePath);
	gtk_widget_destroy(m_pDlg);
}

void CMasterFlatDlg::response_dialog(GtkDialog *pDlg, gint response_id, CMasterFlatDlg *pMe)
{
	if (!pMe->OnResponseDialog(response_id))
		g_signal_stop_emission_by_name(pDlg, "response");
}

bool CMasterFlatDlg::OnResponseDialog(gint response_id)
{
	switch (response_id)
	{
	case GTK_RESPONSE_HELP:
		// Show context help
		g_MainWnd->ShowHelp(GTK_WINDOW(m_pDlg), IDH_MAKE_MASTER_FLAT);
		return false;
	}
	return true;
}

void CMasterFlatDlg::button_clicked(GtkWidget *button, CMasterFlatDlg *pDlg)
{
	pDlg->OnButtonClicked(button);
}

void CMasterFlatDlg::OnButtonClicked(GtkWidget *pBtn)
{
	if (pBtn==m_OptionsBtn)
		EditPreferences();
}

void CMasterFlatDlg::Execute()
{
	int res;
	char msg[256];
	...
	folder = g_Project->GetStr("MasterFlat", "Folder", NULL);
	if (!folder)
		folder = CConfig::GetStr("MasterFlat", "Folder", NULL);
	if (!folder)
	...
	g_free(folder);
	filename = g_Project->GetStr("MasterFlat", "File", "masterflat." FILE_EXTENSION_FITS);
	if (filename) 
	...
	CConfig::SetStr("FlatCorr", "File", m_FilePath);

	gchar *dirpath = g_path_get_dirname(m_FilePath);
	g_Project->SetStr("MasterFlat", "Folder", dirpath);
	CConfig::SetStr("MasterFlat", "Folder", dirpath);
	g_free(dirpath);
	gchar *basename = g_path_get_basename(m_FilePath);
	g_Project->SetStr("MasterFlat", "File", basename);
	g_free(basename);
...
int CMasterFlatDlg::ExecuteProc(CProgressDlg *sender, void *userdata)
{
	return ((CMasterFlatDlg*)userdata)->ProcessFiles(sender);
}

int CMasterFlatDlg::ProcessFiles(CProgressDlg *sender)
{
	int frameid, res;
	char *fpath, *fbase, msg[128];
	GtkTreePath *path;
	CMasterFlatProc mflat;

	m_InFiles = 0;
	sender->Print("------ Make master flat frame ------");

	res = mflat.Open(sender, m_FilePath);
	if (res==0) {
...
void CMasterFlatDlg::EditPreferences(void)
{
	CEditProjectDlg pDlg(GTK_WINDOW(m_pDlg));
	pDlg.Execute(PAGE_MASTER_FLAT);
}
