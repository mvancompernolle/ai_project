...
void chkmany()
{
    ...
    int const n_elt = 40;
    int const mask = 0xff;
    typedef more::gen::unifying_partial_ordering<unsigned int> container;
    ...
    *po.max() = mask;
	...
	more::gen::find_infimums(po.min(), value, preceq,
				std::inserter(S_inf, S_inf.begin()));
	e[i] = po.insert(value);
	for (int j = 0; j < i; ++j) {
	    if (preceq(*e[i], *e[j])) {
		std::cout << *e[i] << " <= " << *e[j] << "; ";
		po.constrain_preceq(e[i], e[j]);
	    }
	    if (preceq(*e[j], *e[i])) {
		std::cout << *e[j] << " <= " << *e[i] << "; ";
		po.constrain_preceq(e[j], e[i]);
	    }
	}
	...
	if (!S_inf.empty() || !S_inf2.empty()) {
	    std::cout << "\n   S_inf = ";
	    std::transform(
		S_inf.begin(), S_inf.end(),
		more::gen::ostream_iterator<unsigned int>(std::cout, ", "),
		more::gen::dereferencer<element>());
	    std::cout << "\n  S_inf2 = ";
	    std::transform(
		S_inf2.begin(), S_inf2.end(),
		more::gen::ostream_iterator<unsigned int>(std::cout, ", "),
		more::gen::dereferencer<element>());
	    std::cout << std::endl;
	}
...
//    typedef std::string string_type;
    ...
    typedef more::gen::unifying_partial_ordering<string_type> container;
    typedef container::element element;
    ...
    *po.max() = "zz_max";
    ...
    po.constrain_preceq(e_alpha0, e_beta0);
    po.constrain_preceq(e_alpha1, e_beta0);
    po.constrain_preceq(e_alpha0, e_beta1);
    po.constrain_preceq(e_alpha1, e_beta1);
    po.constrain_preceq(e_beta0, e_gamma0);
    po.constrain_preceq(e_beta0, e_gamma1);
    po.constrain_preceq(e_beta0, e_gamma2);
    po.constrain_preceq(e_beta1, e_gamma0);
    po.constrain_preceq(e_beta1, e_gamma1);
    po.constrain_preceq(e_beta1, e_gamma2);

    std::list<element> l_infs;
    more::gen::find_infimums(po.min(), string_type("bzeta"),
			    string_less, back_inserter(l_infs));
    std::cout << "Found " << l_infs.size() << " infimums: ";
    std::transform(l_infs.begin(), l_infs.end(),
		   more::gen::ostream_iterator<string_type>(std::cout, ", "),
		   more::gen::dereferencer<element>());
    std::cout << std::endl;
